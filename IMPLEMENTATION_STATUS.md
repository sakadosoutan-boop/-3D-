...
- 表紙のBGMは無し（`SFX.theme=null`）。散策中の楽の音レイヤー（箏・琵琶・笙・龍笛）は既定で切、
  設定「音」の〈楽の音〉または `?music=on` で入。環境音（虫・鳥・せせらぎ）と各モード専用BGMは従来どおり。
- 音源は配信向けに再エンコード済み（ボス戦BGM=96kbpsステレオ／他=64kbpsモノラル、合計38MB→16MB）。
  差し替え時は `bash scripts/optimize-sounds.sh` を実行する。
- 先読みは3段（`SFX.load`）。tier0=今の季節×時刻で鳴る音を即時、tier1以降は同時2本の待ち行列。
