...
### 第2話
収集を `term_card_place`(拾って青い光の輪へ運ぶ配置パズル)へ拡張し、完了後に `seq_206_place_intro` から `seq_206_place_close` の回想問答を追加。御簾/廂の理解を、空間と人間関係の距離へ接続する。配置の説明は `seq_204d`(スポーン前)で行う。

### 第3話
`seq_304c_letter_intro` から `seq_304h_infer_*` を追加。姫君の破れ文を拾い、返歌前に「届いたか分からない言葉」の痛みを推理する。河童戦の動機を、単なる怪異退治ではなく未返信の嘆きの鎮魂へ寄せた。

### 第4話
`seq_409a_opponent_poems` から `seq_409c_sleeve_*` を追加。本番前に右方の歌と袖に忍ばせる短冊を提示し、稽古選択が本番の心構えへつながるようにした。袖の短冊も `utakaiPrepScore` に加算され、直後の `seq_409zx_prep_branch`(しきい値4)で判者の激励/戒めの台詞が分岐する。

### 第5話
`seq_503e_corridor_intro` から `seq_503j_return` を追加。過去フラグに応じて、春の学び拒否、秋の覗き、冬の残留願望、帰る歌が逆さの回廊に映る。ラスボス戦をプレイヤー自身の選択の総決算にする。

### 第6話
病室の読み聞かせ理由を補足する `seq_ed1_wake1b_hospital_reason`(ED1では「栞、お前だったのか」の気づきの**後**に総括として置く) / `seq_ed2_wake1b_hospital_reason`(ED2はぼかした版)と、ED1のフラグ差分 `seq_616a_balanced_*` / `seq_616b_white_*` を追加。True Endは「夢だった」ではなく、「誰かの声が御簾になって届いていた」結末として読めるようにした。回想の間(seq_601系)には種明かし段落を置かない——未到達プレイヤーへのネタバレ防止。
