...
const ST_ICON_FILES={
  ...
  // 波AO: 平安世界(異界)に馴染んで以降(第2話〜)は烏帽子を戴いた描き下ろし立ち絵へ
  hidetora_eboshi:"icons/hidetora_eboshi.webp",
  // 波AO: ED4(常世の婿)——最後に小萩と世界へ残る結末だけ、御簾越しの影ではなく姿の見える描き下ろしへ
...
function stHidetoraIcon(ev){
  ...
  // 波AO: 平安世界では「最初の章(異界に落ちた直後)」を除き烏帽子姿(描き下ろし1枚)で通す
  if(SM&&SM.state&&SM.state.chapterId>=2)return "hidetora_eboshi";
  // 第1話(異界に落ちた直後)は素頭。表情差分(worried/blush)はこの章でのみ効かせる
  const e=ev&&ev.emotes&&ev.emotes.hidetora;
  const bare=(e==="worried"||e==="blush")?"hidetora_"+e:"hidetora_neutral";
