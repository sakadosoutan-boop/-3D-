...
(function(global){
  ...
  const STORY_ENDINGS = Object.freeze({
    ...
    BAD_SYNC: "ED4_SYNC",
    SPOOKY: "ED5_SPOOKY",
    /* 巡り歌合(第2モード)add-on: 第2モード専用の4結末ID。第1モードのED1〜5とは別系統。
       章(story/chapters/)未実装のため、determineEnding が返すIDのみ定義する。
       STORY_ENDING_ENTRY へのエピローグ・マッピングは章実装時に登録する(現状は triggerEnding へ素通し)。 */
    MEGURI_TRUE: "MEGURI_ED1_TRUE",
    MEGURI_DRAW: "MEGURI_ED2_DRAW",
    MEGURI_CURSE: "MEGURI_ED3_CURSE",
    MEGURI_SPOOKY: "MEGURI_ED4_SPOOKY"
  });
  ...
  class StoryManager {
    ...
    /* file:// 対策 — 本体HTMLへ同梱した window.STORY_EMBED = {manifest, chapters:{1:{...}}}
       があれば fetch せずそれを使う。HTTP配信時は従来通り相対fetchする。
       (単一HTML主義との両立: ビルド時に story/chapters/*.json を <script> へ流し込む) */
    embedded(){
      return (global.STORY_EMBED && typeof global.STORY_EMBED === "object") ? global.STORY_EMBED : null;
    }

    async loadManifest(manifestPath = `${this.basePath}chapter_manifest.json`){
      const emb = this.embedded();
      if(emb && emb.manifest){this.manifest = clonePlain(emb.manifest);return this.manifest;}
    ...
    async loadChapter(chapterId){
      const emb = this.embedded();
      let chapter = null;
      if(emb && emb.chapters && emb.chapters[chapterId]){
        chapter = clonePlain(emb.chapters[chapterId]);
      }else{
        if(!this.fetchImpl)throw new Error("StoryManager requires fetch() or options.fetchImpl");
        /* manifest優先でファイル名を解決(QA改善候補2)。未ロード時は従来の命名規則へフォールバック */
        let file = `chapter${chapterId}.json`;
        try{
          const manifest = this.manifest || await this.loadManifest();
          this.manifest = manifest;
          const entry = (manifest.chapters || []).find(ch => ch.chapterId === chapterId);
          if(entry && entry.file)file = entry.file;
        }catch(e){ /* manifest無しでも旧命名で続行 */ }
    ...
    applyEffects(effects){
      if(!effects)return;
      const prevErosion = this.state.params.brainErosion || 0;
      ...
      if(prevErosion < 80 && this.state.params.brainErosion >= 80){
        this.hooks.onErosionWarning(this.state.params.brainErosion, this.snapshot());
      }
      this.hooks.onStateChanged(this.snapshot());
      this.save();
    }
    ...
    determineEnding(requirements = {}){
      ...
      /* === 巡り歌合(第2モード)add-on: 早期分岐 ===
         meguri系フラグ(meguriCount>=1)が立っている巡回時のみ、第2モード専用の4結末を先に判定する。
         meguriCount 未設定(=既存ch1〜6の全ルート)のときはこのブロックを完全に素通しし、
         従来ロジック(ED1〜5)へ一切影響を与えない=既存6章の結末判定は不変。
         数値フラグ(zureFound/yomiSolved/meguriCount)は effects の加算機構で貯まり、
         kaeshiChosen は utakai_meguri の返し歌選択(story_runtime.js)で保存される想定。 */
      if((Number(f.meguriCount) || 0) >= 1){
        const zureFound = Number(f.zureFound) || 0;
        const yomiSolved = Number(f.yomiSolved) || 0;
        const megCount = Number(f.meguriCount) || 0;
        // 蝕(brainErosion)限界 または 巡回過多 → 蝕ED(百夜一夜・第1モードED5と対のSpooky)
        if(p.brainErosion >= 100 || megCount >= 8)return STORY_ENDINGS.MEGURI_SPOOKY;
        // 歌でなく勝敗で押し切った(裁く) → 呪ED(詠み残し・Bad)
        if(f.kaeshiChosen === "sabaku")return STORY_ENDINGS.MEGURI_CURSE;
        // ズレ4・読み3・返し歌「返す」・雅50以上 → 真ED(明けの歌)
        if(zureFound >= 4 && yomiSolved >= 3 && f.kaeshiChosen === "kaesu" && p.fantasySynchro >= 50){
          return STORY_ENDINGS.MEGURI_TRUE;
        }
        // それ以外 → 和ED(引き分けの宵)
        return STORY_ENDINGS.MEGURI_DRAW;
      }
      ...
      /* Perfectフラグはミニゲーム側評価(result.flags)で付与される予定。
         requirePerfect:false を渡すと勝利フラグ(utakaiWon/oniWon)で代用できる
         (ミニゲーム評価が未接続の間、True Endが構造的に到達不能になるのを防ぐ) */
      const requirePerfect = requirements.requirePerfect !== false;
      const perfectOk = requirePerfect
        ? (f.utakaiPerfect && f.oniPerfect)
        : (f.utakaiWon && f.oniWon);
      if(realityHigh && fantasyHigh && erosionLow && perfectOk){
        return STORY_ENDINGS.TRUE;
      }

      /* ED4(常世の婿)は「小萩を理想像として選んだ」者の結末。
