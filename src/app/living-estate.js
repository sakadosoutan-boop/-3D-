...
(function livingEstateSystem(){
  ...
  // [fix]来客・使者・牛車の到着は日のあるうち(朝/昼/夕)のみ。夜は屋敷を静かに閉じるため到着なし
  const ARRIVAL_BY_TIME={dawn:"messenger",day:"visitor",dusk:"oxCart",night:null};
  ...
  // [fix]屋敷生活系の通知には出所を示すタグを付け、何の機能のメッセージか(=「日課」パネル)が分かるようにする
  const ESTATE_TAG="〔屋敷の一日〕";
  function notify(message,ms){say(ESTATE_TAG+" "+message,ms);}
  ...
  function refreshPanel(){
    ...
    Object.keys(ARRIVALS).forEach(type=>{
      ...
      const isNight=!!(APP&&APP.time==="night"); // [fix]夜は来客対応なし
      const stateText=state.arrival&&state.arrival.type===type?"進行中":isNight?"夜は休み":state.nextArrival===type?"まもなく":"散策中に開始";
      button.append(element("span","",def.label),element("small","",stateText));
      button.disabled=!!state.arrival||!walkMode()||isNight;button.onclick=()=>triggerArrival(type);
  ...
  function triggerArrival(type){
    ...
    if(APP&&APP.time==="night"){notify("来客の対応は日のあるうちに。夜は屋敷を静かに閉じています",2600);return false;} // [fix]夜は来客対応をしない
    if(APP&&APP.gisshaCarry&&APP.gisshaCarry.active){notify("牛車運びが終わってから来客を迎えよう",2200);return false;}
  ...
  function finishArrival(silent){
    ...
    if(!silent)notify("来客の対応がひと段落した",2200);
