...
(function(){
  ...
  function start(options){
    stored=readState();const config=options&&typeof options==="object"?options:{};let candidates=SCENARIOS.slice();if(config.scenarioId){const forced=byId(SCENARIOS,config.scenarioId);if(forced)candidates=[forced].concat(candidates.filter(item=>item.id!==forced.id));}const rounds=shuffle(candidates,config.seed).slice(0,ROUNDS).map(makeRound);session={rounds,index:0,score:0,phase:"motif",answered:false,selectedIndex:null,answer:null,complete:false};stored.plays+=1;writeState();if(modal&&modal.classList.contains("is-open"))render();return getState();
  }
