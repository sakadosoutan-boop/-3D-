...
最終更新: 2026-06-16。**これが最新の正本**。`NEXT_CHAT_GITHUB_HANDOFF.md` / `CLAUDE_HANDOFF.md` / `PROJECT_MAP.md` は過去経緯の参考に留める（記載のHEADやブランチ指示は古い）。

## 0. 最重要
- 正本は GitHub **origin/main**。作業前と push 前に必ず `git fetch origin` し、Claude/Codex 双方の最新を取り込む。ローカルの古いHTMLを正本にしない。
- 現在の origin/main: **`ac51313`**（公開済み）。Claudeの作業ブランチ `claude/friendly-albattani-9po8fx` は main と一致。
- 公開: main へ push すると GitHub Actions（`.github/workflows/pages.yml`）が自動デプロイ。
  - 公開URL: https://sakadosoutan-boop.github.io/-3D-/ （反映確認はキャッシュ回避 `?v=ac51313`）
...
**大（独立ブランチ・設計から）**
1. **垣間見の本改修**: 4進入経路→3観察地点へ整理、モード中だけ建物衝突、母屋正面を封鎖し妻戸経路から、2無音2有音、クリア後に未達成地点へ挑戦。**几帳の操作**も同時に。
2. 絵巻断片・和歌短冊の収集ループ（ローカル/オンライン順位と設計整合）。

**中**
3. オンライン全国順位: 静的Pagesではバックエンド必須（Supabase/Firebase/Cloudflare）。匿名ID・スコア検証・不正対策・通信失敗時動作を決める。※ローカル順位は実装済み。
4. 壁・柱の当たり判定（建物すり抜け）。「建物に入れる」体験を壊さない設計が要る。
5. 雅楽風BGM、AO/ライトマップベイク、絵巻物風ミニマップ本格化、朝を含むmingの音の作り込み。

**小**
6. `BENCHMARKS.md` に最新の `?fps=1` 数値を記録（要実機）。`sounds/CREDITS.md` の権利確定。
