...
最終更新: 2026-06-17（第2セッション）。**これが最新の正本**。`NEXT_CHAT_GITHUB_HANDOFF.md` / `CLAUDE_HANDOFF.md` / `PROJECT_MAP.md` は過去経緯の参考に留める（記載のHEADやブランチ指示は古い）。

## 0. 最重要
- 正本は GitHub **origin/main**。作業前と push 前に必ず `git fetch origin` し、Claude/Codex 双方の最新を取り込む。ローカルの古いHTMLを正本にしない。
- 現在の origin/main: **`5c5a771`**（公開済み・2026-06-17 第2セッション デプロイ完了）。
...
1. **垣間見の本改修（部分実装済み）**:
   ...
   - ✅ 几帳の操作（`5ce5540`で実装済み）
   - ✅ 几帳越しに方向チェック（南側・北向き）追加（`5c5a771`）
   - ✅ 垣間見成功時に「再挑戦/散策」選択（`5c5a771`）
   - ❌ 4進入経路→3観察地点へ整理（現在4スポット: 几帳越し/東格子/北壁代/妻戸先）
   - ❌ 母屋正面を封鎖し妻戸経路のみに
   - ❌ 2無音2有音（2スポットは女房なし、2スポットは女房ガード）
2. ✅ 和歌短冊100枚の収集ループ（`5c5a771`で百人一首100首に拡張）。絵巻断片は未実装。
3. ❌ 灯籠流しイベント（夏・夜専用、池面に流す演出、複雑なため後回し）
...
**小**
7. `BENCHMARKS.md` に最新の `?fps=1` 数値を記録（要実機）。`sounds/CREDITS.md` はプレースホルダー更新済み・**個別URL確定は未完**。

## 4b. 今セッションで実装済み（`5c5a771`）
- ✅ 垣間見/几帳越し: 南側・北向き方向チェック追加
- ✅ questHud最小化: タイトル横ボタンでミッション詳細をトグル
- ✅ 垣間見成功時の選択: 再挑戦/散策ボタン
- ✅ タイムアタックランク表: 全称号表を結果下部に表示
- ✅ 百人一首100首: 春夏秋冬各25首（計100首）
- ✅ 朝の静寂: dawn SOUNDSCAPE を bed:null・超低頻度化
- ✅ 朝靄フォグ: dawn時 density=0.020、fog色白みがかりに
- ✅ ミニマップ烏帽子アイコン: 向き付き烏帽子形状（黒・金縁）
- ✅ 和歌フォント教科書体: YuKyokasho を優先指定
- ✅ 解説既登録: 図鑑登録済みなら opacity:0.70・2.2秒フェード
- ✅ 女性篇会話速度: 4.5秒に延長

## 5. 次セッションへ貼る短縮指示

```text
HANDOFF_LATEST.md を最初に全文読む。正本は GitHub origin/main（現在 5c5a771）。
