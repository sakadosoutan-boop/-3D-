...
- **武器刷新**: 呪印→**破魔の札**(自分中心の衝撃波AoE、クールタイム4s、`spawnShockwave`)、弓矢→**破魔の矢**(矢が飛んで着弾判定、`spawnArrow`/`taijiArrows`を描画ループで更新)。効果音も別々(`seArrowShot/seArrowHit/seFuda`)。
- **敵HPバー**: `makeHpBar`(Sprite+CanvasTexture)を各霊の頭上に。被弾で`updateSpiritHp`が更新。
- **特殊妖怪の攻撃**: taiji中、配置妖怪に近づくと特徴的攻撃(雪女=冷気で鈍足`slowUntil`+ダメージ、狐火=火傷、河童=引込み大ダメージ、提灯=怪火、人魂=憑依)。
- HUDは前回から下部中央(`#taijiHud`)＋HPバー。ダッシュ閾値0.8、スマホで設定/ヘルプ表示。

### スマホ統一・四脚門・物の怪退治の戦闘化（2026-06）
- **スマホ/タブレット統一**: `@media(max-width:760px)`で`#tbGfx,#tbHelp`(設定・ヘルプ)を非表示にしていたのを解除(トップバーは横スクロールなので収まる)。ダッシュ判定を`joy>0.9→0.8`に緩め小型ジョイでも届くように。
- **四脚門**: 開口(4幅)に対し門幅(~4.8)が広く収まり切らず変だったため、東壁南セグメントを`wallSeg(48,W,36.5)→wallSeg(45,W,38)`にして開口を z=8.5〜15.5 に拡張。向きは前回の通り築地と平行(rotation 0)。
- **物の怪退治モードの戦闘化**:
  - HUDを画面下部中央へ移動(`#taijiHud` bottom-center)。トップバーと被らない。**HPバー**(`#tjHpWrap/tjHpFill/tjHpLabel`)を追加。
  - 怨霊(kekkai)が**プレイヤーへ接近**(aggro 24〜32、種類別速度)。接触(<1.7)で**ダメージ**(通常9/強14、0.9sクールダウン)、`#dangerVignette`点滅。HP0で`endTaiji(true)`=敗北画面。
  - **配置妖怪(creatures.yokai: 人魂/河童/提灯/狐火/雪女)を攻撃可能**に。taijiShootで弓のミス時にyokaiメッシュもraycastし、当たれば9秒間祓う(`yk.dispelledUntil`で再表示まで隠す)。呪印は周囲もまとめて祓う。結果に「追い祓い」数を表示。
- 注: 退治の勝利条件は従来通り全kekkai退散。配置妖怪はボーナス。
